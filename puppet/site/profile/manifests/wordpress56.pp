class profile::wordpress56 (
  ...
  package {['rh-php56-php-mysqlnd', 'sclo-php56-php-pecl-redis']:
    ensure  => installed,
  ...
  Exec {
    path => '/opt/rh/rh-php56/root/usr/bin:/opt/rh/rh-php56/root/usr/sbin:/usr/local/sbin:/usr/local/bin:/sbin:/bin:/usr/sbin:/usr/bin:/root/bin',
  }

  exec {'install composer':
    cwd         => '/usr/local/bin',
    environment => 'HOME=/usr/local/bin',
    command     => 'curl -sS https://getcomposer.org/installer | /opt/rh/rh-php56/root/usr/bin/php && mv composer.phar /usr/local/bin/composer',
    creates     => '/usr/local/bin/composer',
    require     => Package['rh-php56-php'],
  }
